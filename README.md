...
> **Status: Phase 2 (Parity pages) — in progress; milestones 1–2 shipped.** All three Hub content types render as static pages: **236 articles, 5 datasets, and 13 apps (518 routes)** with the Figma design system, canonical URLs, and JSON-LD. Cross-type Related Content links are live. All gates green in CI (16 a11y checks across 8 routes × 2 color schemes); continuous deploys from `main`: **https://copperhead-hub-20.netlify.app/researchhub/** (crawling blocked until launch). Phase 2 complete (every URL-contract route, 538 prerendered, componentized per Figma block) and **Phase 3 under way**: the stakeholder filter experience is live — one-click type chips, author filter + author names on cards, topic/year filters, search highlighting, all URL-synced. CMS consolidated to one type per concept (`*home` singletons retired into `pages` entries; type deletion scheduled after 2026-07-31, see `docs/cms-consolidation.md`). Next: site-wide search including document bodies (Pagefind + `@icjia/pdf-search-index`). Last updated: July 11, 2026.
