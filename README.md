...
> **Status: Phase 2 (Parity pages) — in progress; milestones 1–2 shipped.** All three Hub content types render as static pages: **236 articles, 5 datasets, and 13 apps (518 routes)** with the Figma design system, canonical URLs, and JSON-LD. Cross-type Related Content links are live. All gates green in CI (16 a11y checks across 8 routes × 2 color schemes); continuous deploys from `main`: **https://copperhead-hub-20.netlify.app/researchhub/** (crawling blocked until launch). **Phases 0–3 complete.** Every URL-contract route prerendered (~540 routes), the stakeholder filter experience live (type chips, author filter, highlighting, URL-synced), and `/search` reaches **inside published PDF/Office documents** (build-time extraction via `@icjia/pdf-search-index` → Pagefind stubs → grouped results, no `unsafe-eval`). CMS consolidated to one type per concept (type deletion scheduled after 2026-07-31, `docs/cms-consolidation.md`). **Phases 0–5 substantially complete.** Parity infrastructure (246/266 archived URLs resolve, all misses accounted for), search inside documents, per-template Lighthouse budgets, CSP + security headers, 30 automated a11y checks, dependency posture clean at runtime, and `docs/runbook.md` with the full launch-day checklist. Remaining before launch: content authoring + cutover sync, manual a11y walkthrough, freeze rehearsal, design sign-off, and the main-site proxy flip (Phase 6). Last updated: July 11, 2026.
